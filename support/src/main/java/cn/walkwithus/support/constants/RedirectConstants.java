package cn.walkwithus.support.constants;

/**
 * @author yangtao.lyt
 * @version $Id: RedirectConstants, v 0.1 2016-01-26 19:55 yangtao.lyt Exp $
 */
public interface RedirectConstants {


    String R_WEB_HOME = "redirect:/";
    String R_WEB_REQUIRE = "redirect:/require";
    String R_WEB_LOGIN = "redirect:/login";
    String R_WEB_REGISTER = "redirect:/register";

    String R_ACTIVITY_HOME_PRE = "redirect:/activity/home?id=";
    String R_ACTIVITY_ACCOUNTINFO_PRE = "redirect:/acl/activity/accountinfo?id=";
    String R_ACTIVITY_MEMBER_PRE = "redirect:/acl/activity/member?activityId=";

    String R_TEAM_HOME_PRE = "redirect:/team/home?id=";
    String R_TEAM_ACCOUNTINFO_PRE = "redirect:/acl/team/accountinfo?id=";
    String R_TEAM_MEMBER_PRE = "redirect:/acl/team/member?teamId=";







}
